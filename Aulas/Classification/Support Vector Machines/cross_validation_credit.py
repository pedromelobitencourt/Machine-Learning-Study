...
for i in range(30):
    ...
    svm = SVC(kernel='rbf', C=2.0) # parameters got from tuning
...
results = pd.DataFrame({'SVM': svm_results})
...
print(shapiro(svm_results)) # if pvalue < alpha: It rejects the null hypothesis (it's not in a normal distribution). Otherwise, it accepts it
